...
use super::{device::ResolvedInput, AudioError, SAMPLE_RATE_HZ};
...
fn ingest<S: IntoMonoF32>(
    ...
    let resampled = if in_rate == SAMPLE_RATE_HZ {
        mono
    } else {
    ...
    while acc.len() >= chunk_samples {
        ...
        if tx.try_send(chunk).is_err() {
            tracing::warn!("audio: dropping chunk — consumer is not keeping up");
        }
    }
}
