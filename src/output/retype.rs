...
//!
//! Ports `daemon_service.py:73-109` from the Python project. Given a stream of
...
/// What [`RetypeState::diff`] emits — a target for a [`KeyboardSink`].
...
/// A sink that the orchestration layer pushes [`RetypeStep`]s into. Phase 6
/// implementations: enigo, ydotool, clipboard.
pub trait KeyboardSink: Send {
    fn apply(&mut self, step: RetypeStep<'_>) -> std::io::Result<()>;
}

/// A no-op sink. Useful in tests that just want to drive the state machine.
#[derive(Debug, Default)]
pub struct NullSink;

impl KeyboardSink for NullSink {
    fn apply(&mut self, _step: RetypeStep<'_>) -> std::io::Result<()> {
        Ok(())
    }
}

/// A sink that records every step. Used by integration tests to verify the
/// state machine produced the expected keystroke sequence.
#[derive(Debug, Default)]
...
impl RecordingSink {
    /// Simulate the cumulative effect of all recorded steps on an initially-
    /// empty terminal. The result is what the user would actually see.
